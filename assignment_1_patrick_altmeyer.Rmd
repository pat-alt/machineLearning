...
```{r plot-error-rates}
library(ggplot2)
p <- ggplot(data = error_rates[block_size_ratio==0.05 & N==max(N)], aes(x=log(n), y=error_rate, colour=estimator)) +
  geom_point() +
  geom_line() +
  facet_grid(
    rows = vars(eps),
...
```{r}
library(ggplot2)
p <- ggplot(data = error_rates[block_size_ratio==0.05 & N==max(N)], aes(x=log(n), y=mae, colour=estimator)) +
  geom_point() +
  geom_line() +
