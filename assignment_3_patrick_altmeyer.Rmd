...
First let us look at the class of all circles:
...
We can first consider this problem graphically like we did in class, where we recursively increase $n$ and check if it is still feasible to shatter the $n$ points. As you can see below, we can shatter the points through circles of varying radii for $n=3$. For $n=4$ the shadow coefficient is $S_{\mathcal{A}}(4)=14<2^4$. In the picture below no sets can be formed including only points that lie on "opposite" sides of the group of four (pairs $\{A,C\}$ and $\{B,D\}$).
...
is a subset of the class of all circles, hence we know for sure that it VC dimension can be at most 3. To test if it is actually 3 we can just test of $n=3$ points can be shattered. The picture below shows that this is indeed possible.
