...
class SmallestCubeClassifier():
  ...
  def transform(self,X):
    self.predicted = np.array([int(i.all()) for i in (X >= bounds[0]) & (X <= bounds[1])])
...
d <- round(exp(1:4))
n <- round(exp(4:8))
J <- 100 # number of independent test sets
...
```{r prob-error, fig.cap="Probability of error of the two classifiers for different sample sizes and dimensions."}
library(ggplot2)
output <- readRDS(file="data/erm.rds")
ggplot(data=output[,.(prob_error=mean(error)),by=.(classif,n,d)], aes(x = log(n), y=prob_error, colour=factor(d), linetype=factor(classif))) +
  geom_line() +
  geom_point() +
